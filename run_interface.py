...
from rm_interface.interface import Window
from reflection_machine.aggregators import v5
from reflection_machine.modules import m0a, m1, m2b
...
class NielsWindow(Window):
    def startRMWindow(self):
        ...
        try:
            ...
            print("RM ERROR: Case description could not be interpreted.")
            return
        try:
            ...
            print("RM ERROR: Support message could not be interpreted.")
            return
        try:
            abcd = "_abcd" #1 = a, 2 = b, 3 = c, 4 = d
            user = self.cases.iloc[self.case_id][abcd[self.choice]].strip().lower()
        except:
            print("RM ERROR: User diagnosis could not be interpreted.")
